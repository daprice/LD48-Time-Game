...
MyGame = ig.Game.extend({
	...
	finishTimewarp: function() {
		...
		for (var i = 0; i < this.timetravelerInstances.length; i++) {
			this.spawnEntity('EntityTimetraveler', 295 - (i * 17), 375, {frames:this.timetravelerInstances[i].frames, warpOnFrame: this.timetravelerInstances[i].warpOnFrame});
		}
		
		//spawn a new player timetraveler
		this.spawnEntity('EntityTimetraveler', 295 - (this.timetravelerInstances.length * 17), 375, {name:'player'});
