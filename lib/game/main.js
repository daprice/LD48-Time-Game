...
MyGame = ig.Game.extend({
	...
	init: function() {
		...
		//temporary code to make it timewarp every 10 seconds
		setInterval(function(){this.timewarp();}.bind(this), 10000);
	},
	...
	finishTimewarp: function() {
		...
		for (var i = 0; i < this.timetravelerInstances.length; i++) {
			this.spawnEntity('EntityTimetraveler', 72 - (i * 17), 72, {frames:this.timetravelerInstances[i].frames, warpOnFrame: this.timetravelerInstances[i].warpOnFrame});
		}
		
		//spawn a new player timetraveler
		this.spawnEntity('EntityTimetraveler', 72 - (this.timetravelerInstances.length * 17), 72, {name:'player'});
...
// up by a factor of 2
ig.main( '#canvas', MyGame, FRAME_RATE, 320, 240, 2 );
